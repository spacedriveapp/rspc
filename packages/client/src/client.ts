...
export class Client<TProcedures extends ProceduresDef> {
  ...
  async query<K extends TProcedures['queries']['key'] & string>(
    ...
    try {
      return await this.transport.doRequest('query', keyAndInput[0], keyAndInput[1])
    } catch (err) {
      if (this.onError) {
  ...
  async mutation<K extends TProcedures['mutations']['key'] & string>(
    keyAndInput: [key: K, ...input: _inferProcedureHandlerInput<TProcedures, 'mutations', K>]
  ): Promise<inferMutationResult<TProcedures, K>> {
    try {
      return await this.transport.doRequest('mutation', keyAndInput[0], keyAndInput[1])
    } catch (err) {
      if (this.onError) {
        this.onError(err as RSPCError)
      }
  ...
  >(
    keyAndInput: [K, ..._inferProcedureHandlerInput<TProcedures, 'subscriptions', K>],
    ...
    try {
      ...
      this.transport.doRequest('subscription', keyAndInput[0], [subscriptionId, keyAndInput[1]])
